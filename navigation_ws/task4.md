...
要改地图就改 `src/main.cpp` 的 `build_maps()`：
...
代码我拆了三层：`grid_map` 管"哪格能走"，`astar` 管"怎么找路"，`main` 管造地图和画图。三层互不知道对方内部怎么实现——换渲染不用动算法，换算法不用动地图。
...
- **4 邻域代价正好 90**：从 (2,2) 到 (57,37) 横竖各要走 55+35=90 步，一步代价 1，所以就是 90。8 邻域能斜走，代价降到 71.8，**少了 20%**，路径点数从 91 降到 60（不再是一格一格的锯齿）。所以能斜走就得让它斜走。
- **展开格数反而是 8 邻域多**（559 vs 91），这不是"8 邻域更慢"：4 邻域的路径贴着地图上边和右边走，能搜的区域本来就窄；8 邻域是斜着穿过去，搜索面是宽的一条带，碰到的格子自然多。**展开数和路径代价得一起看，单看一个会得出错误结论。**

另外踩了两个坑：一是斜走会穿墙（加了 `diagonal_allowed` 检查），二是同一个格子会在优先队列里躺好几份（用惰性删除跳过过期数据）。
