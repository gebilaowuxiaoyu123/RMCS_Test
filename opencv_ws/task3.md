...
六个参数都可以在命令行上敲，**不传就用默认值**：

```bash
./build/contour_detect <图片> <输出前缀> <最小周长> <Canny低阀> <Canny高阀> <模糊核>
# 例：默认参数
./build/contour_detect images/fruits.jpg output/fruits
# 例：只要主体轮廓、不要碎线
./build/contour_detect images/fruits.jpg output/fruits_clean 100 100 300 7
# 例：细节全要、宁可碎一点
./build/contour_detect images/fruits.jpg output/fruits_more  15  30  90 3
```
...
**1. 最小周长 `min_length`（命令行第 3 个）——最安全，先动这个**

它只是“事后过滤”，不改边缘检测本身，所以随时可以来回试。

- 调到 **100~150** → 碎线大幅减少，只剩长轮廓，画面立刻干净
- 调到 **10** → 几乎全保留，会满屏碎线

**2. Canny 双阀值——影响“线条多少”最直接**

代码里是 `cv::Canny(blurred, edges, canny_low, canny_high);`（现在可以从命令行传）。

| 想要的效果 | 怎么改 |
|---|---|
| 只留主要轮廓、干净 | 两个都调大，例如 `100 300` |
| 细节全都要、宁可碎 | 两个都调小，例如 `30 90` |
| 线条还是太多 | 优先调**高阀值**（第二个数），它管“多强的边才算边” |
| 轮廓断断续续 | 调小**低阀值**，让弱边缘也能接上强边缘 |

经验：高阀值约等于低阀值的 2~3 倍最稳。

**3. 高斯模糊核——管噪点**

代码里是 `cv::GaussianBlur(gray, blurred, cv::Size(blur_size, blur_size), 1.5);`。

- 改 **7 或 9** → 抹掉更多细碎纹理 → 线条少而稳（代价：细小轮廓可能被一起糊掉）
- 改 **3** → 细节多，但噪点也一起进来
...
**5. `findContours` 的模式——想只要外轮廓时用**

- 现在是 `RETR_LIST` = 所有轮廓（含内部细节，所以蝴蝶翅服的花纹都在）
- 改 **`RETR_EXTERNAL`** = 只留最外层一圈，内部细节全丢，就是那种“简笔画”效果
- `CHAIN_APPROX_NONE`（每个点都留）改成 **`CHAIN_APPROX_SIMPLE`**（只留拐点）→ 线条更折线化、更省内存

**6. 输入图本身**

图片先缩到宽 800 以内（太大线条又碎又慢）。另像 `web_1.jpg` 那张逆光窗户，本身就没几条稳定的边，是素材问题不是参数问题——换图比调参有用。

**调一组对比看效果**（同一张 fruits.jpg）：

| 参数 | 检出轮廓 | 过滤后 | 效果 |
|---|---|---|---|
| 默认（周长 40、Canny 60/180、模糊 5） | 82 | 33 | 细节适度，可用的基准 |
| 严格（周长 100、Canny 100/300、模糊 7） | 7 | 2 | 只剩最大的几块，干净但偏少 |
| 宽松（周长 15、Canny 30/90、模糊 3） | 529 | 340 | 细节全在，但碎线满屏 |

**一句话**：嫌乱就先把最小周长调大，还不够再把 Canny 高阀值调大；嫌缺细节就把两个阀值都调小、模糊核调小。
